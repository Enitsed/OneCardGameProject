...
/*	private static final String SEPA = "|";
	...
	*/
	...
	public String searchGrade=null;
		...
		try{
			...
			while(selfThread == currThread){
				...
				switch (cmd) {
					...
					{
						Sound("src/login.wav");
						JOptionPane.showConfirmDialog(null, "로그인이 완료 되었습니다.", "메세지", JOptionPane.CLOSED_OPTION, JOptionPane.INFORMATION_MESSAGE);
					...
					case MEMBERSHIP_SUCCESS :{
						JOptionPane.showConfirmDialog(null, "회원가입에 성공했습니다.", "메세지", JOptionPane.CLOSED_OPTION, JOptionPane.INFORMATION_MESSAGE);
						lg.registerFrame.dispose();
						break;
					}
					case ROOMLIST : {
						//StringTokenizer st_test2 = new StringTokenizer(st.nextToken(), SEPA);//new StringTokenizer("USER1|USER2|USER3|USER4", SEPA);
					...
					case JOINROOM_FAIL : {
						JOptionPane.showConfirmDialog(null, "방 입장에 실패했습니다.", "메세지", JOptionPane.CLOSED_OPTION, JOptionPane.ERROR_MESSAGE);
						break;
					}
					case SENDWORD : {
                        String id = st.nextToken();
					...
					case LOGOUT_SUCCESS:
					{
						JOptionPane.showConfirmDialog(null, "로그아웃 되셨습니다!", "메세지", JOptionPane.CLOSED_OPTION, JOptionPane.ERROR_MESSAGE);
						System.exit(0);
						break;
					}
					case LOGIN_FAIL:
					{
						JOptionPane.showConfirmDialog(null, "비밀번호랑 아이디를 확인하세요.", "메세지", JOptionPane.CLOSED_OPTION, JOptionPane.ERROR_MESSAGE);
						break;
					}
					case SELECT_SHAPE :
					{
					...
					case CLOSECHATROOM_SUCCESS : {
			            ...
			            UiChattingRoom = null;
			            JOptionPane.showConfirmDialog(null, "채팅방에서 나가셨습니다.", "메세지", JOptionPane.CLOSED_OPTION, JOptionPane.INFORMATION_MESSAGE);
					...
					case WIN_LOSE_DTO_UPDATE: {
			               ...
			               if(UiChattingRoom != null) {
			                  UiChattingRoom.infUpdate(dto.getWins(), dto.getLoses(),dto.getRank_score(),dto.getRank(),dto.getGrade(),dto.getWinRate());
			               }
			               break;
			            }
				}	
	...
	public void closeChatRoom() {
	    int ans = JOptionPane.showConfirmDialog(null, "정말로 나가시겠습니까?", "메세지", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
	...
	public void login(String id, String password){
		int ans = JOptionPane.showConfirmDialog(null, "'" + id + "'로 로그인을 하시겠습니까?", "메세지", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
		if(ans == JOptionPane.YES_OPTION){
			buf.setLength(0);
			...
			JOptionPane.showConfirmDialog(null, "접속을 종료합니다.", "메세지", JOptionPane.CLOSED_OPTION, JOptionPane.INFORMATION_MESSAGE);
			System.exit(0);
		}
	}
	...
	public void register(String string, String string2, String string3, String string4, String string5, String string6, char[] cs) {
		buf.setLength(0);
		buf.append(MEMBERSHIP + SEPA + string + SEPA + string2+ SEPA + string3+ SEPA + string4 + SEPA + string5 + SEPA + string6 + SEPA  + String.valueOf(cs));
		send(buf.toString());
	}
